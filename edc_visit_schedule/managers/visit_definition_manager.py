...
class VisitDefinitionManager(models.Manager):
    ...
    def next_visit_definition(self, **kwargs):
        ...
        if kwargs.get('visit_definition'):
            ...
            if super(VisitDefinitionManager, self).filter(schedule_group=schedule_group, code=code):
                ...
                raise ValueError(
                    '{} method {} cannot determine the visit_definition given '
                    ' schedule_group=\'{}\' and code=\'{}\''.format(
                        self.__name__, inspect.stack()[0][3], schedule_group, code))
        else:
        ...
        visit_definitions = super(VisitDefinitionManager, self).filter(
            schedule_group=visit_definition.schedule_group).exclude(id=visit_definition.id).order_by('time_point')
    ...
    def list_all_for_model(self, registered_subject, model_name):
        ...
        ScheduleGroup = apps.get_model('edc_visit_schedule', 'schedulegroup')
        if ScheduleGroup.objects.filter(member__content_type_map__model=model_name):
    ...
    def relativedelta_from_base(self, **kwargs):
        ...
        if kwargs.get('visit_definition'):
            ...
            raise AttributeError('{} method {} requires a visit_definition instance'.format(
